...
- ✅ **Notification listener** path for Play-Store-safe distribution, with generic bank-app push parsing plus wallet/card-app phrases, peer-payment `sent you`/`got paid` income, bank-specific card-charge/debit-card merchant extraction, CAD/AUD/CHF notification currencies, comma-decimal amount formats, and Google Pay/Samsung Pay package variants
...
- ✅ Rule engine with **633 merchant seed rules** across 22 categories (beta.17+R-05 — expanded from 44 curated rules with AI-categorized patterns plus reviewed priority-90 catalog batches; rules auto-refresh on app upgrade)
...
- ✅ **Statement import + CSV export** for Excel and bank-statement interop — preview detected fields and sample rows, choose the destination account, then import TMOAP/Athar CSVs, common statement CSVs/TSVs with description/amount, split debit/credit, or debit-credit indicator columns, OFX/QFX, or MT940 files; export annual data for your accountant
...
| G-12 | Bank statement / CSV / OFX / QFX / MT940 import wizard | ◐ | 3 days | CSV/TSV delimiter + header auto-detect, debit/credit indicator support, preview/confirm, OFX/QFX, MT940, and destination-account selection shipped; manual mapping and richer row editing still planned |
