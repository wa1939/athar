...
- ✅ Rule engine with **686 merchant seed rules** across 22 categories (beta.17+R-05 — expanded from 44 curated rules with AI-categorized patterns plus reviewed priority-90 catalog/private-audit batches; rules auto-refresh on app upgrade)
...
- ⏳ Device E2E, screenshot UI audit, and notification tap-through still need a physical Android device or a working accelerated emulator. On 2026-06-13 the local `AtharPixelQaApi35Arm` AVD exited with an arm64-on-x86_64 architecture mismatch, while `AtharPixelQaApi35` launched headlessly with software acceleration but stayed `offline` in ADB, preventing install, launch, screenshots, and tap-through.
