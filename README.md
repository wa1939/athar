...
- ✅ **G-5** — Localization (full): locale picker, DataStore persistence, `attachBaseContext` Configuration override + Compose `LocalLayoutDirection` provider. **~280 user-facing strings extracted** to `values/` (Arabic default) + `values-en/` across app:onboarding, feature:today, feature:trends, feature:plan, feature:settings. Verified end-to-end: switching to English flips text to English and layout direction to LTR (dot indicators, FAB anchor, back arrows). ViewModel-emitted strings (4 error fallbacks, backup status, auto-detected recurring suggestion notes) remain Arabic — flagged with TODO; a follow-up will inject Context or move to sealed error types.

### In flight / remaining

| ID | Feature | Status | Effort | Notes |
|---|---|---|---|---|
| G-5b | Localization — ViewModel string extraction | ⏳ | 0.5 day | ~6 ViewModel-emitted strings (error fallbacks, backup status, auto-suggestion notes) need Context injection or sealed error type so Activity can resolve via `stringResource()` |
