...
- ✅ **Notification listener** path for Play-Store-safe distribution, with generic bank-app push parsing plus wallet/card-app phrases, peer-payment `sent you`/`got paid` income, posted refund/reversal/chargeback income, normalized ATM/cash-withdrawal, bank-fee, debt-payment, utility-bill, salary-income, subscription-payment, insurance-premium, rent-payment, mobile-recharge, traffic-fine, government-service, parking-payment, toll-payment, and transit-fare labels, bank-specific card-charge/debit-card merchant extraction, merchant-first charge/card-transaction copy, status-prefixed posted transaction merchant hints, amount-adjacent trailing merchant/biller extraction, structured `Merchant:`/`Merchant Name:`/`Location:`/`Card acceptor:`/`Sender:`/`Recipient:`/`Biller:`/`Payer:`/`Receiver:` labels, balance-aware transaction amount selection, clear balance-after extraction, balance/credit-state ignores, card/account admin ignores, spending-summary/recap ignores, reward/cashback amount guards, payment/money-request guards, OTP/payment-authorization and temporary-authorization-hold ignores, broader global ISO and regional-symbol notification currencies, comma-decimal amount formats, scheduled-payment, withdrawal-limit, fee-schedule, debt-reminder, utility-bill-reminder, recurring-expense-reminder, telecom-recharge-promo, public-service-reminder, mobility-payment-reminder, salary-transfer financing-offer, and marketing-discount ignores, Google Pay/Samsung Pay package variants, and more MENA/APAC/Australia/US finance package coverage
...
- ✅ **Bulk categorize with AI (beta.18+)** — copy the in-app AI prompt, export non-transfer PENDING/DISMISSED/uncategorized transactions to CSV, grouped so repeated merchants appear first with a `merchant_group_count` and row-level `category_options`, run it through ChatGPT/Claude, import the filled file or saved fenced `csv` response back. Choose full-context export when raw SMS text helps categorization, or private export when you want the same import contract with `raw_body` blank. A single unambiguous compatible category choice can update blank peers in the same repeated-merchant group, incompatible expense/income category mismatches are skipped with visible reason counts, and each unambiguous type-safe merchant→category decision records an exact local rule so future SMS for the same normalized merchant auto-categorize. No API keys, no cloud round-trip — your personal merchant library grows permanently
...
| G-11 | Broader notification handlers | ◐ | 3 days | Generic parser, wallet/card phrases, peer-payment/global-currency handlers, regional currency symbols, localized comma-decimal amount parsing, bank-specific card-charge/debit-card merchant extraction, merchant-first charge/card-transaction/POS transaction parsing, status-prefix merchant hints, amount-adjacent trailing merchant/biller extraction, structured merchant/location/party label-field extraction, balance-aware amount selection, clear balance-after extraction, ATM/bank-fee/debt-payment/utility-bill/salary-income/recurring-expense/telecom-recharge/public-service/mobility-payment shared labels, balance/credit/admin/summary/payment-request ignores, reward/cashback guards, OTP/payment-authorization and authorization-hold ignores, scheduled-payment/repayment-reminder/utility-bill-reminder/recurring-expense-reminder/telecom-recharge-promo/public-service-reminder/mobility-payment-reminder/salary-transfer-financing-offer/marketing-discount ignores, and broader package coverage shipped; more real bank copy remains |
