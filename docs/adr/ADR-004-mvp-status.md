...
|---|---|---|
| M-14 XLSX importer | Superseded post-beta.24 | At MVP time, CSV was the supported migration path and XLSX was deferred to avoid Apache POI weight. Post-beta.24 G-12 now supports transaction-grid OpenXML `.xlsx` workbooks through a hand-rolled ZIP/XML reader while keeping non-transaction TMOAP sheets out of statement exchange. |
