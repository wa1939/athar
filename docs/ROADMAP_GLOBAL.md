...
**Status:** CSV import now auto-detects the original Athar/TMOAP layout plus common statement layouts (`description`/`amount`, split `debit`/`credit`, optional ISO currency). OFX/QFX files use the same preview-confirm path for `DTPOSTED`, `TRNAMT`, `NAME`/`MEMO`, `TRNTYPE`, `CURDEF`, and `FITID`; MT940 files now map `:61:` transaction records, `:86:` details, and statement currency from balance records. Settings also lets users choose the active destination account before confirming an import.
...
| 4 | G-12 import wizard | 3 days | CSV auto-detect + preview/confirm, OFX/QFX, MT940, and destination-account selection shipped; manual mapping and richer multi-currency statement UX remain |
