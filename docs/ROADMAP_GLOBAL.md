...
**Status:** CSV import now auto-detects the original Athar/TMOAP layout plus common statement layouts (`description`/`amount`, split `debit`/`credit`, optional ISO currency). OFX/QFX files use the same preview-confirm path for `DTPOSTED`, `TRNAMT`, `NAME`/`MEMO`, `TRNTYPE`, `CURDEF`, and `FITID`; MT940 files now map `:61:` transaction records, `:86:` details, and statement currency from balance records.
**Gap:** Account selection, manual column mapping, and richer multi-currency/account UX are still missing. Users on every continent need to be able to drag in their statement and verify rows before commit.
**Fix:**
...
| 4 | G-12 import wizard | 3 days | CSV auto-detect + preview/confirm, OFX/QFX, and MT940 first slices shipped; manual mapping, account selection, and richer multi-currency statement UX remain |
