...
**Status:** Store-safe listener is wired, generic parsing works, localized comma-decimal amounts are handled, and the first app-specific phrase/package follow-up has shipped; more bank-specific handlers remain.
...
| QA-01 (P0 before release) | Device E2E, screenshot UI audit, and notification tap-through could not run on 2026-06-13 because no Android device was attached and the AVD cannot boot without hardware acceleration. This remained true during `dev/today-goals-nudge`, `dev/bill-reminders`, `dev/sender-matching-reliability`, `dev/notification-phrase-coverage`, `dev/csv-import-preview-confirm`, `dev/curated-seed-rules-batch`, `dev/notification-app-handlers`, `dev/ofx-qfx-import-preview`, `dev/mt940-import-preview`, `dev/import-account-selection`, `dev/csv-delimiter-detection`, `dev/localized-amount-parsing`, and `dev/curated-seed-rules-batch-2` validation: `adb devices -l` returned no attached devices. | Enable virtualization / Android Emulator hypervisor driver or connect a physical Android device with USB debugging, then run the smoke flows listed in [`docs/RECOVERY_2026-06-13.md`](RECOVERY_2026-06-13.md). |
...
| 2 | G-8 manual transaction UX follow-ups | 1.5 days | Voice entry + local encrypted receipt photos; recent merchants/quick-add already shipped |
| 3 | G-11 bank-specific notification handlers follow-up | 2–3 days | Generic localized amount parsing and first app-handler batch shipped; continue with more real Wise/Revolut/Chase/Capital One/Mercury notification copy |
| 4 | G-12 import wizard | 3 days | CSV/TSV delimiter + header auto-detect, preview/confirm, OFX/QFX, MT940, and destination-account selection shipped; manual mapping and richer multi-currency statement UX remain |
