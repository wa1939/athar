...
**Status:** CSV import now auto-detects the original Athar/TMOAP layout plus common statement layouts (`description`/`amount`, split `debit`/`credit`, optional ISO currency), and OFX/QFX files now use the same preview-confirm path for `DTPOSTED`, `TRNAMT`, `NAME`/`MEMO`, `TRNTYPE`, `CURDEF`, and `FITID`.
**Gap:** MT940, account selection, manual column mapping, and richer multi-currency/account UX are still missing. Users on every continent need to be able to drag in their statement and verify rows before commit.
**Fix:**
...
| 4 | G-12 import wizard | 3 days | CSV auto-detect + preview/confirm and OFX/QFX first slice shipped; manual mapping, account selection, MT940, and richer multi-currency statement UX remain |
