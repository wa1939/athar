...
**Status:** CSV import now auto-detects the original Athar/TMOAP layout plus common statement layouts (`description`/`amount`, split `debit`/`credit`, optional ISO currency), then previews detected columns and sample rows before the user confirms.
**Gap:** OFX, QFX, MT940, account selection, and manual column mapping are still missing. Users on every continent need to be able to drag in their statement and verify rows before commit.
**Fix:**
...
| **post-beta.24** | **G-12 CSV import preview/confirm** — Settings now previews detected columns, importable/skipped counts, sample rows, and the first skipped row before committing a CSV import. | TMOAP and bank-statement migration should not be a blind write. The preview uses the same parser plan as import, caches the selected bytes until confirm, and lets the user cancel before anything is inserted. Manual mapping and OFX/QFX/MT940 remain future G-12 work. | [G-12 preview](specs/G-12-csv-import-preview-confirm.md) |
...
| 4 | G-12 import wizard | 3 days | CSV auto-detect + preview/confirm shipped; manual mapping, account selection, OFX/QFX/MT940, and multi-currency statement UX remain |
