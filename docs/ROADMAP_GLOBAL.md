...
| QA-01 (P0 before release) | Device E2E, screenshot UI audit, and notification tap-through could not run on 2026-06-13 because no Android device was attached and the AVD cannot boot without hardware acceleration. This remained true during `dev/today-goals-nudge` and `dev/bill-reminders` validation: `adb devices -l` returned no attached devices. | Enable virtualization / Android Emulator hypervisor driver or connect a physical Android device with USB debugging, then run the smoke flows listed in [`docs/RECOVERY_2026-06-13.md`](RECOVERY_2026-06-13.md). |
