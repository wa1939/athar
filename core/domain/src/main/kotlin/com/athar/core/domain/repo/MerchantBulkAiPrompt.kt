...
Your job: fill in the category_id column for every row you can classify confidently. Prefer one of the ids shown in that row's category_options column; Athar also tolerates the full copied option string such as cat-coffee=Coffee / قهوة, but the id alone is best. For repeated rows with the same merchant_normalized, prioritize the lowest merchant_group_rank values first and use merchant_group_share_permille to understand how much of the cleanup backlog a group represents. You may fill only the first representative row when every blank peer should inherit the same category. If a row in the same merchant group needs a different category, fill that row explicitly too; Athar will not train a learned rule for conflicting or incompatible-type groups. If category_options is missing or incomplete, use only these category ids:
