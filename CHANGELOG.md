...
- Store-safe notification parsing now lets more known finance app packages from MENA, Australia, India/Southeast Asia, and the US reach the existing conservative parser without allowing arbitrary notification packages.
...
- Curated seed rules now include 135 more active high-confidence merchants from catalog and aggregate-only private-audit review, bringing `seed_rules.json` from 551 to 686 rules with asset tests for category validity, duplicate patterns, and the R-05 batches.
