...
- Curated seed rules now include 139 more active high-confidence merchants from catalog and aggregate-only private-audit review, bringing `seed_rules.json` from 551 to 690 rules with asset tests for category validity, duplicate patterns, and the R-05 batches. The latest slice adds Arabic public-platform labels for Ejar, Mobily, Gonsure, and Tameeni, and corrects the existing Arabic Ejar platform phrase to rent.
