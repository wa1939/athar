...
| H-01 | All-Transactions / History screen — full-list view with merchant search + category filter + date range + bulk re-categorize | P1 | 2 | M-01, M-04 | New `feature/history` module (or Settings entry); list shows all CONFIRMED transactions across time; search box filters by merchant substring; category-filter chip row; tap row opens existing EditTransactionSheet; "select multiple → reassign category" bulk action; works on 10k+ rows without jank (LazyColumn + paged DAO). ✅ shipped beta.12; bulk re-categorize deferred to H-02. |
| R-01 | Backfill propagation latency — Today/History flows don't reflect inserts during a long SMS backfill until app restart | P0 | 1 | F-07 | When SmsBackfillService imports >100 messages, the flood of `pipeline.process(event)` coroutines saturates Room's invalidation tracker; Today screen shows empty state until activity recreation. Fix: route ingestion through a Channel with batched flush (e.g., 50 events per Room transaction), debounce flow emissions, OR add a one-shot `refreshNow()` call on Today VM init. Repro: fresh install → grant SMS → "Rescan and clean" → land on Today → empty until restart. |
| R-02 | Auto-categorization expansion via AI triage — feed bulk SMS exports to ChatGPT/Claude using `docs/AI_SMS_TRIAGE_PROMPT.md`, paste JSON output back, expand seed_rules + seed_merchant_catalog + parser templates | P1 | 1 (per batch) | M-04 | The prompt teaches an external AI the schema; output JSON gets merged into seed_rules.json and per-bank template files. Each batch is additive; corpus tests prevent regressions. Future: in-app version that runs against the local Activity Log without leaving the device. |
| R-03 | Recurring rule create-confirmation sheet with category + cadence picker | P1 | 1 | G-3 | Tapping "Create rule" on an auto-detected suggestion currently creates the rule with default values (MONTHLY cadence, dayOfMonth from median, categoryId=null). Should instead open a bottom sheet pre-filled with the auto-detected values where the user confirms cadence (Monthly/Weekly/Yearly), assigns a category, optionally edits the day-of-month, and optionally toggles `isActive`. Improves trust: the user sees exactly what rule will be created before committing, and the rule lands with a real category so future SMS auto-confirm correctly. |
| R-04 | "Active subscriptions" framing in Recurring Rules screen — rename header from "Rules" to "Subscriptions", group active vs inactive, add quick-deactivate swipe, surface total monthly subscription cost | P2 | 0.5 | G-3 | User mental model is "subscriptions" (Netflix, gym, rent) more than "recurring rules". Rename UI, add a header card showing total monthly subscription expense, group rules under "Active" + "Paused" sections, swipe-left on Active to deactivate (with undo). |
